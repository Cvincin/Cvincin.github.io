...
function generateItem() {
    ...
    item.innerHTML = `
        ...
        <img src="images/examples/${currentItem.name}.png" alt="${currentItem.name}" class="item-image" oncontextmenu="return false;">
    `;
...
function touchStart(event) {
    ...
    event.preventDefault();  // 阻止默认行为（如长按弹出菜单）
    // 在触摸开始时禁用图片上的长按菜单
    const target = event.target;
    if (target.tagName === 'IMG') {
        target.addEventListener('contextmenu', function(e) {
            e.preventDefault(); // 防止右键菜单出现
        });
    }
...
function touchMove(event) {
    event.preventDefault();  // 阻止触摸移动引起的滚动
...
function touchEnd(event) {
    event.preventDefault();  // 防止触摸结束时页面滚动
    ...
    if (bin && bin.classList.contains("bin")) {
        ...
        handleDrop(category, binCategory, bin);
    }
}
